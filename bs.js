...
console.log('[Nion Bot] >> BOT STARTED');
const promos = ''
...
//
user.setOptions({
	token: 'fd34b0f4e08f6defb09d942319c32f99a6e31d3b5ea0c59ada677b5bab71f3ace6d6b63b939ef041f3764' // token ot страницы ( не обязательно )
});
...
vk.setOptions({ token: '9b91db99fc89aa84432fc4a5a3fb8199eb9ae6699faa6d263c540b0196b56bb4e82cf6feee0d94c3f18bc', pollingGroupId:155310910 });
...
[🌹]На данный момент проходит Event,в честь "8го марта".Каждый Pay-Day(час)вы получаете 1розу,которую можно обменять на новый бизнес или на 5000$`);
...
[🥇]Топ-топ игроков
[🔦]Ограбить-ограбить банк

[🌹]EVENT:
[🌹]Подарить[ид]-подарить игроку розу
[🌹]Обменять[кол-во]-обменять розы на деньги
...
const ping = utils.random(300);
	await message.send(`>>[📖]Bot Info:

    »[🤖]Name Project: Nion Bot
	»[🖥]Version:1.0
	»[🔑]Load: ${percent}%
	»[📶]Ping: ${ping}
	»[💰]Баланс казны: ${kazna.kbalanc}$
	»[🎩]Зарегистрировано: ${kazna.acc}$
`
);
...
cmd.hear(/^(?:раздача)$/i, async (message, bot) => {
		if(message.senderId !== 324503169 && message.senderId !== 299908038) return;
...
Приятной игры,с уваженим Admin's Team👓` });
					vk.api.messages.send({ user_id: 324503169, message: `[😜] >> [id324503169|Одмэн], я выдал игроку ([id${user.id}|${user.tag}]) - 200.000 на баланс.\n\nНа данный момент, его баланс составляет:\n${user.balance}$`})
				});
			});
			user.api.wall.openComments({
				owner_id: -179170041,
	...
	await bot(`[👾]ADMIN-PANEL:Successfully`);
});
...
cmd.hear(/^(?:eval)\s([^]+)$/i, async (message, bot) => {
	...
	if(message.senderId !== 324503169 && message.senderId !== 299908038 && message.senderId !== 324503169 && message.senderId !== 324503169 && message.senderId !== 324503169) return;
...
text += `>>[✒]Имя: ${message.user.tag} || [👓] Префикс: [${message.user.tityl}]\n`;	
text += `>>[👾]GAME ID: ${message.user.uid}\n`;
text += `>>[🍗]Состояние голода: ${message.user.golod}%\n`;
text += `>>[💰]Наличка: ${message.user.balance}\n`;
text += `>>[👑]Короны: ${message.user.rating}\n`;
text += `>>[🌹]Розы: ${message.user.roza}\n`;
...
cmd.hear(/^(?:money)\s([0-9]+)\s(.*)$/i, async (message, bot) => {
	message.user.ybralcmd +=1;
	message.args[2] = message.args[2].replace(/(\.|\,)/ig, '');
	message.args[2] = message.args[2].replace(/(к|k)/ig, '000');
	message.args[2] = message.args[2].replace(/(м|m)/ig, '000000');
...
cmd.hear(/^(?:подарить)\s(.*)$/i, async (message, bot) => { 
		if(message.user.balance < 100000)return bot (`[🌹]Роза стоит 100.000$!`);
		...
		message.user.balance -=100000;
				...
				await bot(`[🌹]Вы успешно подарили розу игроку: "${user.tag}"`);
		if(user.notifications) vk.api.messages.send({ user_id: user.id, message: `
[🌹]Игрок: ${message.user.tag} подарил вам розу!С 8марта[🌹]` });
});
...
cmd.hear(/^(?:mmoney)\s(.*)$/i, async (message, bot) => {
	message.args[1] = message.args[1].replace(/(\.|\,)/ig, '');
	message.args[1] = message.args[1].replace(/(к|k)/ig, '000');
	message.args[1] = message.args[1].replace(/(м|m)/ig, '000000');	
...
message.user.floder += 1;
  ...
  vk.api.messages.send({ user_id: zz.id, message: `[👾РАССЫЛКА]:${message.args[1]}`}); 
 }); 
 			let people = 0;
        for(let id in users) {
        ...
        return message.send(`[👾]ADMIN-PANEL:Successfully`);
...
Для покупки введите "Бизнесы [номер]"`);
	...
	if(message.user.balance < sell.cost) return bot(`[🏢]недостаточно денег`);
	if(message.user.roza < sell.roza) return bot(`[🏢]Данный бизнес можно обменять только на 1.000🌹`);
	else if(message.user.balance >= message.args[1])
	{
		message.user.balance -= sell.cost;
		message.user.roza -= sell.roza;
...
cmd.hear(/^(?:промокод)\s([^]+)$/i, async (message, bot) => {
	...
	if(!promo[message.args[1]]) return message.reply(`>>[🎁]Данный промокод несуществует.`, {attachments: 'photo-179170041_456239034'});
...
cmd.hear(/^(?:dell)\s(.*)$/i, async (message, bot) => { 
message.user.ybralcmd +=1;
		if(message.user.alvl < 4)return;
...
cmd.hear(/^(?:exit)$/i, async (message, bot) => {
	...
	await bot(`	[Nion Bot] >> LOADING,PLACE WAIT!`);
	await message.send(`[Nion Bot] >> LOADING,PLACE WAIT!`);
	await message.send(`[Nion Bot] >> LOADING,PLACE WAIT!`);
	await message.send(`[Nion Bot] >> LOADING,PLACE WAIT!`);
	await message.send(`[Nion Bot] >> LOADING,PLACE WAIT!`);
	await message.send(`[Nion Bot] >> LOADING,PLACE WAIT!`);
    ...
    await message.send(`[Nion Bot] >> BOT OFFLINE`);
